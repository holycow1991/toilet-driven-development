...
function printReport(findings, targetDir) {
  ...
  if (findings.length === 0) {
    console.log(c('green', '  ✨ Wow. No lazy comments found. You\'re either very clean or very good at hiding them.\n'));
    return 0;
  }
  ...
  const severity = total <= 5  ? c('green',  '🟢 Manageable')
                 : total <= 15 ? c('yellow', '🟡 Concerning')
                 : total <= 30 ? c('red',    '🔴 Yikes')
                               : c('red',    c('bold', '💀 ABANDON SHIP'));

  console.log('─'.repeat(50));
  console.log(c('bold', `  🚽 Flush Report`));
  console.log(`  Total bathroom breaks needed : ${c('bold', String(total))}`);
  console.log(`  Files affected               : ${c('bold', String(fileCount))}`);
